...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
     ...
     */
    private static int leftIndex(int i) { return 2 * i; }

    /**
     * Returns the index of the node to the right of the node at i.
     */
    private static int rightIndex(int i) { return 2 * i + 1; }

    /**
     * Returns the index of the node that is the parent of the node at i.
     */
    private static int parentIndex(int i) { return i / 2; }
    ...
    private void sink(int index) {
        ...
        int cmp2 = min(right, index);
        if(cmp1 == index && cmp2 == index) return;
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; i++) {
            ...
            if (item.equals(curr.item())) {
                double temp = curr.priority();
                Node nodeNew = new Node(item, priority);
                contents[i] = nodeNew;
